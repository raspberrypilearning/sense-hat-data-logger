...
- For each piece of data you want to log you will need a **Boolean** (one that can either be **True** or **False**) variable to your settings section. In the code below, these have been capitalised to make them stand out. For now, they have been set to `True`.
...
- You then need to update your `file_setup` function to only append the headings that are set to `True`
...
- The final change to make is to the `get_sense_data` function, which is a similar change to the one made to the `file_setup` function. Your code which did look like this:
  ...
  You can find a complete code listing [here](resources/Sense_Logger_v3.py).
